...
from src.live.kalshi.offline_training import (  # noqa: E402
    ...
    DEFAULT_LINEAR_SVM_ARTIFACTS_ROOT,
    DEFAULT_LINEAR_SVM_C_VALUES,
    DEFAULT_LINEAR_SVM_MAX_ITER,
    DEFAULT_LINEAR_SVM_SAMPLE_SIZE,
    DEFAULT_LINEAR_SVM_TOL,
    ...
    evaluate_walk_forward_linear_svm,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train and evaluate the KXBTC15M L1-regularized Linear SVM pipeline."
    )
    ...
    parser.add_argument("--linear-svm-sample-size", type=int, default=DEFAULT_LINEAR_SVM_SAMPLE_SIZE)
    parser.add_argument("--linear-svm-c-values", type=float, nargs="+", default=list(DEFAULT_LINEAR_SVM_C_VALUES))
    parser.add_argument("--linear-svm-max-iter", type=int, default=DEFAULT_LINEAR_SVM_MAX_ITER)
    parser.add_argument("--linear-svm-tol", type=float, default=DEFAULT_LINEAR_SVM_TOL)
    parser.add_argument(
    ...
    artifacts_dir = resolve_artifacts_dir(
        args.artifacts_root,
        args.run_name,
        default_root=DEFAULT_LINEAR_SVM_ARTIFACTS_ROOT,
    )
    ...
    train_subset = sample_train_subset(train_df, args.linear_svm_sample_size)
    print(f"Searching Linear SVM C over {len(args.linear_svm_c_values)} candidates on {len(train_subset)} sampled rows...")
    best_params, search_history = run_linear_svm_search(
        train_subset,
        validation_df,
        c_values=tuple(float(value) for value in args.linear_svm_c_values),
        max_iter=args.linear_svm_max_iter,
        tol=args.linear_svm_tol,
        progress_desc=None if args.no_progress else "Linear SVM search",
    )
    write_json(
        artifacts_dir / "linear_svm" / "search_history.json",
        {
    ...
    print("Fitting final Linear SVM model on the full train split...")
    artifact, training_metrics = train_linear_svm_model(train_df, validation_df, best_params)
    save_linear_svm_artifacts(artifacts_dir, artifact, training_metrics)

    print("Calibrating validation probabilities...")
    validation_raw = linear_svm_raw_predictions(artifact, validation_df)
    ...
    test_raw = linear_svm_raw_predictions(artifact, test_df)
    test_calibrated = apply_calibration_to_predictions(artifacts_dir, test_raw, model_family="linear_svm")
    ...
    if args.skip_walk_forward:
        ...
        walk_forward = evaluate_walk_forward_linear_svm(
            dataset_cache_dir,
    ...
    if args.skip_latest_publish:
        ...
        latest_dir = publish_latest_artifacts(artifacts_dir, model_family="linear_svm")
