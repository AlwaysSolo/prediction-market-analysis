...
from src.live.kalshi.offline_training import (  # noqa: E402
    ...
    DEFAULT_LASSO_ARTIFACTS_ROOT,
    DEFAULT_LASSO_C_VALUES,
    DEFAULT_LASSO_MAX_ITER,
    DEFAULT_LASSO_SAMPLE_SIZE,
    DEFAULT_LASSO_TOL,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train and evaluate the KXBTC15M LASSO pipeline (L1-regularized logistic regression)."
    )
    ...
    parser.add_argument("--lasso-sample-size", type=int, default=DEFAULT_LASSO_SAMPLE_SIZE)
    parser.add_argument("--lasso-c-values", type=float, nargs="+", default=list(DEFAULT_LASSO_C_VALUES))
    parser.add_argument("--lasso-max-iter", type=int, default=DEFAULT_LASSO_MAX_ITER)
    parser.add_argument("--lasso-tol", type=float, default=DEFAULT_LASSO_TOL)
    parser.add_argument(
    ...
    artifacts_dir = resolve_artifacts_dir(
        args.artifacts_root,
        args.run_name,
        default_root=DEFAULT_LASSO_ARTIFACTS_ROOT,
    )
    ...
    train_subset = sample_train_subset(train_df, args.lasso_sample_size)
    print(f"Searching LASSO C over {len(args.lasso_c_values)} candidates on {len(train_subset)} sampled rows...")
    best_params, search_history = run_lasso_search(
        train_subset,
        validation_df,
        c_values=tuple(float(value) for value in args.lasso_c_values),
        max_iter=args.lasso_max_iter,
        tol=args.lasso_tol,
        progress_desc=None if args.no_progress else "LASSO search",
    )
    write_json(
        artifacts_dir / "lasso" / "search_history.json",
        {
    ...
    print("Fitting final LASSO model on the full train split...")
    artifact, training_metrics = train_lasso_model(train_df, validation_df, best_params)
    save_lasso_artifacts(artifacts_dir, artifact, training_metrics)
    ...
    test_raw = lasso_raw_predictions(artifact, test_df)
    test_calibrated = apply_calibration_to_predictions(artifacts_dir, test_raw, model_family="lasso")
    ...
    if args.skip_walk_forward:
        ...
        walk_forward = evaluate_walk_forward_lasso(
            dataset_cache_dir,
    ...
    if args.skip_latest_publish:
        ...
        latest_dir = publish_latest_artifacts(artifacts_dir, model_family="lasso")
