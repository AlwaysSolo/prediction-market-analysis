...
from src.live.kalshi.offline_training import (  # noqa: E402
    ...
    DEFAULT_BAGGED_LASSO_ARTIFACTS_ROOT,
    DEFAULT_BAGGED_LASSO_C_VALUES,
    DEFAULT_BAGGED_LASSO_MAX_ITER,
    DEFAULT_BAGGED_LASSO_MAX_SAMPLES,
    DEFAULT_BAGGED_LASSO_N_ESTIMATORS,
    DEFAULT_BAGGED_LASSO_N_JOBS,
    DEFAULT_BAGGED_LASSO_SAMPLE_SIZE,
    DEFAULT_BAGGED_LASSO_TOL,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train and evaluate the KXBTC15M Bagged LASSO pipeline (bootstrap-aggregated L1 logistic regression)."
    )
    ...
    parser.add_argument("--bagged-lasso-sample-size", type=int, default=DEFAULT_BAGGED_LASSO_SAMPLE_SIZE)
    parser.add_argument("--bagged-lasso-c-values", type=float, nargs="+", default=list(DEFAULT_BAGGED_LASSO_C_VALUES))
    parser.add_argument("--bagged-lasso-n-estimators", type=int, default=DEFAULT_BAGGED_LASSO_N_ESTIMATORS)
    parser.add_argument(
        "--bagged-lasso-max-samples",
        type=float,
        default=DEFAULT_BAGGED_LASSO_MAX_SAMPLES,
        help="Bootstrap sample fraction used by each bagged estimator.",
    )
    parser.add_argument("--bagged-lasso-max-iter", type=int, default=DEFAULT_BAGGED_LASSO_MAX_ITER)
    parser.add_argument("--bagged-lasso-tol", type=float, default=DEFAULT_BAGGED_LASSO_TOL)
    parser.add_argument("--bagged-lasso-n-jobs", type=int, default=DEFAULT_BAGGED_LASSO_N_JOBS)
    parser.add_argument(
    ...
    artifacts_dir = resolve_artifacts_dir(
        args.artifacts_root,
        args.run_name,
        default_root=DEFAULT_BAGGED_LASSO_ARTIFACTS_ROOT,
    )
    ...
    train_subset = sample_train_subset(train_df, args.bagged_lasso_sample_size)
    print(
        "Searching Bagged LASSO C over "
        f"{len(args.bagged_lasso_c_values)} candidates on {len(train_subset)} sampled rows..."
    )
    best_params, search_history = run_bagged_lasso_search(
        train_subset,
        validation_df,
        c_values=tuple(float(value) for value in args.bagged_lasso_c_values),
        n_estimators=args.bagged_lasso_n_estimators,
        max_samples=args.bagged_lasso_max_samples,
        max_iter=args.bagged_lasso_max_iter,
        tol=args.bagged_lasso_tol,
        n_jobs=args.bagged_lasso_n_jobs,
        progress_desc=None if args.no_progress else "Bagged LASSO search",
    )
    write_json(
        artifacts_dir / "bagged_lasso" / "search_history.json",
        {
    ...
    print("Fitting final Bagged LASSO model on the full train split...")
    artifact, training_metrics = train_bagged_lasso_model(train_df, validation_df, best_params)
    save_bagged_lasso_artifacts(artifacts_dir, artifact, training_metrics)

    print("Calibrating validation probabilities...")
    validation_raw = bagged_lasso_raw_predictions(artifact, validation_df)
    ...
    test_raw = bagged_lasso_raw_predictions(artifact, test_df)
    test_calibrated = apply_calibration_to_predictions(artifacts_dir, test_raw, model_family="bagged_lasso")
    ...
    if args.skip_walk_forward:
        ...
        walk_forward = evaluate_walk_forward_bagged_lasso(
            dataset_cache_dir,
    ...
    if args.skip_latest_publish:
        ...
        latest_dir = publish_latest_artifacts(artifacts_dir, model_family="bagged_lasso")
